...
void BoxObject::update(float deltaTime)
{
	...
	{
		//std::cout << "x: " << GestureManager::getInstance()->getTouchEventData()->getXpos() << " y: " << GestureManager::getInstance()->getTouchEventData()->getYpos() << std::endl;
	}
}
...
void BoxObject::onGesture(GestureListener::GestureEvent evt)
{
	...
	case GestureListener::GestureEvent::HOLD:
		if (GestureManager::getInstance()->getTouchEventData() != NULL) 
		{
			m_positionX = GestureManager::getInstance()->getTouchEventData()->getXpos();
			m_positionY = GestureManager::getInstance()->getTouchEventData()->getYpos();
		}
		break;
	};
}
